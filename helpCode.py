...
class Client(object):
    ...
    * Use disconnect() to disconnect from the broker
    ...
    def _packet_read(self):
        ...
        # Finally, free the memory and reset everything to starting conditions.steve
        if self._in_packet['command'] == 0:
            try:
                if self._ssl:
                    ...
                    c=int.from_bytes(command,byteorder='big')
                    a=c&15
                    print("\nreceived command ",hex(c)," flags binary ",bin(a))#steve
    
                    #print("received flags binary ",bin(a))#steve
                    b0=command
                    #print("b0 -",b0)#steve
            except socket.error as err:
        ...
        if self._in_packet['have_remaining'] == 0:
            ...
            while True:
                try:
                    ...
                    byte = byte[0]
                    #print("total bytes received=",byte)
        ...
        buf=b0+b1+self._in_packet['packet']#steve create received data
        print("received data",buf)#steve
        print("received data decimal",list(buf),"\n")#steve
    ...
    def _packet_write(self):
        ...
        while self._current_out_packet:
            ...
            a=packet['packet'][0]
            c=a&15
            print("sending command ",hex(a)," sending flags =",c)                              
            print("sending ",packet['packet'][packet['pos']:])#steve
    ...
    def _packet_queue(self, command, packet, mid, qos, info=None):
        ...
        #t_temp ="Sending Packet "+ mpkt
        #print("sending data ",mpkt) #steve
        p=len(mpkt["packet"])
        print("length of packet is",p) #steve
...
#steve
        if sys.version_info[0] >= 3:
            message.topic = message.topic.decode('utf-8',"ignore")
...
class WebsocketWrapper:
    ...
    def _buffered_read(self, length):
        ...
        print("read buffer=   ",self._readbuffer)
